package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IAvatarImageListener
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(_selectedColorIndex >= var_320.length)
         {
            return "";
         }
         var petColor:IPetColor = var_320[_selectedColorIndex] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_628[var_1613] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(_selectedColorIndex < 0 || _selectedColorIndex > var_320.length)
         {
      ...
      override public function init() : void
      {
         ...
         var_920 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_320 = [];
         var_628 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_920,this);
               if(_loc11_)
               {
                  var_628 = _loc11_.breeds;
                  var_320 = _loc11_.colors;
                  if(var_320 == null)
                  {
                     var_320 = [];
                  }
         ...
         while(_loc6_ < var_628.length)
         {
            ...
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_920,_loc12_.id),getRaceLocalizationKey(var_920,_loc12_.id)));
            _loc6_++;
         }
         var_592.add(var_920,_loc5_);
         ...
         _loc5_ = var_592.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_320)
         {
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_592.getWithIndex(0),page,p);
      }
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_592.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(_selectedColorIndex >= 0 && _selectedColorIndex < var_320.length)
         {
            _loc7_ = var_320[_selectedColorIndex] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_628[var_1613] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_920,_loc3_.id,_loc2_,AvatarScaleType.const_56,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_270,AvatarAction.const_667);
