package com.sulake.habbo.friendlist
{
   ...
   public class FriendRequestsView implements ITabView, IFriendRequestsView
   {
      ...
      private static const const_1453:String = "no_reqs_info";
      ...
      private function onAcceptAllButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            if(_loc4_.state != FriendRequest.const_330 && _loc4_.state != FriendRequest.const_344)
            {
               _loc3_.addAcceptedRequest(_loc4_.requestId);
               _loc4_.state = FriendRequest.const_330;
      ...
      {
         var_1506 = IContainerButtonWindow(param1.findChildByName("accept_all_but"));
         var_1505 = IContainerButtonWindow(param1.findChildByName("reject_all_but"));
         var_1505.procedure = onDismissAllButtonClick;
         var_1506.procedure = onAcceptAllButtonClick;
         refreshButtons();
      }
      ...
      private function onDismissAllButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         for each(_loc4_ in _friendList.friendRequests.requests)
         {
            if(_loc4_.state != FriendRequest.const_330 && _loc4_.state != FriendRequest.const_344)
            {
               _loc4_.state = FriendRequest.const_344;
               refreshRequestEntry(_loc4_);
               if(_friendList && false)
               {
                  _friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_294,_loc4_.requestId));
               }
            }
         }
         refresh();
      ...
      private function refreshButtons() : void
      {
         ...
         setEnabled(var_1506,_loc1_);
         setEnabled(var_1505,_loc1_);
      }
      ...
      public function acceptRequest(param1:int) : void
      {
         ...
         _loc2_.state = FriendRequest.const_330;
      ...
      public function declineRequest(param1:int) : void
      {
         ...
         _loc2_.state = FriendRequest.const_344;
         var _loc3_:DeclineBuddyMessageComposer = new DeclineBuddyMessageComposer();
         ...
         if(_friendList && false)
         {
            _friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_294,param1));
         }
      }
   }
}
